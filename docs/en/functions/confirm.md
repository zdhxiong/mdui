...
The `confirm` function wraps the [`<mdui-dialog>`](/en/docs/2/components/dialog) component and replaces the native `window.confirm` function. With this function, you can open a confirmation dialog without writing any HTML.
...
The function takes an [Options](#api-options) object and returns a Promise. If the dialog is closed by clicking the confirm button, the Promise resolves. If the dialog is closed in any other way, the Promise is rejected.
  ...
  <tbody>
    ...
    <tr>
      <td colspan="3">
        ...
        <p>The <a href="/en/docs/2/functions/dialog"><code>dialog()</code></a>, <a href="/en/docs/2/functions/alert"><code>alert()</code></a>, <code>confirm()</code>, and <a href="/en/docs/2/functions/prompt"><code>prompt()</code></a> functions share the same queue if their queue names match.</p>
      </td>
    </tr>
