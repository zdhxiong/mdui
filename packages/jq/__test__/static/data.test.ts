...
<div class="intro" data-key="val"></div>
`;
    });

    it('$.data(element, key, value)', () => {
      const intro = $('.intro')[0];
      ...
      assert.sameOrderedMembers(
        $.data(intro, 'array') as number[],
        testArrayData,
      );

      // $.data(element, key, undefined) 相当于 $.data(element, key)
      assert.sameOrderedMembers(
        $.data(intro, 'array', undefined) as number[],
        testArrayData,
      );
      assert.sameOrderedMembers(
        $.data(intro, 'array') as number[],
