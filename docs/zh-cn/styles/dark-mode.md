...
mdui 的所有组件都支持深色模式，并且支持根据操作系统的设置自动切换主题。
...
如果要使用深色模式，只需在 `<html>` 标签上添加 `mdui-theme-dark` 类即可：
...
也可以在页面的不同部分使用不同的主题。例如下面的示例，在 `<html>` 上设置深色模式，但在页面中的一个 `<div>` 上添加了 `mdui-theme-light` 类，这样该 div 中的元素将显示为浅色模式，而页面其余部分则为深色模式：
...
下面示例将浅色模式下的页面背景设为纯白，文字设为纯黑；且深色模式下页面背景设为纯黑，文字设为纯白：
