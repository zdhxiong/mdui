...
function show(event, $ripple) {
    ...
    // 点击位置坐标
    const touchPosition = event instanceof TouchEvent && event.touches.length
        ? event.touches[0]
        : event;
...
mdui.dialog = function (options) {
    ...
    // Dialog 的 HTML
    const HTML = `<div class="mdui-dialog ${options.cssClass}"></div>` +
        (options.title
