...
标签组件用于辅助用户输入信息、进行选择、筛选内容或执行相关操作。
...
使用 `variant` 属性设置标签的形状。标签有 4 种形状，可以根据用途选择：
...
添加 `icon`、`end-icon` 属性，可以分别在标签左侧、右侧添加 Material Icons 图标。也可以通过 `icon`、`end-icon` slot 在标签左侧、右侧添加元素。
...
添加 `href` 属性，可以使标签变为链接，此时还可以使用这些和链接相关的属性：`download`、`target`、`rel`。
...
添加 `disabled` 属性可以禁用标签；添加 `loading` 属性可以为标签添加加载中状态。
...
标签被选中后，`selected` 属性变为 `true`。也可以通过添加 `selected` 属性，使标签默认处于选中状态。
...
添加 `deletable` 属性后，标签右侧会出现一个删除图标。点击该图标会触发 `delete` 事件。您可以通过 `delete-icon` 属性指定删除图标的 Material Icons 图标名，或者通过 `delete-icon` slot 指定删除图标的元素。
