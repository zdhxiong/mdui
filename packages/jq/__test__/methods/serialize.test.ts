...
`;
    ...
    it('.serialize(): string', () => {
      ...
      assert.equal(
        $('#frame input[name="check"]').serialize(),
      ...
      assert.deepEqual(
        $('#frame input[name="text"]').serialize(),
        'text=text&text=text&text=text',
      );

      assert.deepEqual($('#frame .form').serialize(), 'text=text&text=text');
    });
  });
};
